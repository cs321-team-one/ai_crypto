...
if (Meteor.isServer) {
    ...
    describe('News', () => {
        ...
        it('has a correct prediction', () => {
            News.find().forEach((article, i) => {
                let articlePrediction = fakeNewsPredictionData[i] === 1 ? 'BUY' : 'SELL';
                expect(article.prediction).to.equal(articlePrediction);
            });
        });
        ...
        it('has a guid', () => {
            News.find().forEach((article) => {
                expect(article).to.have.property('guid');
            });
        });

        it('has a published_on', () => {
            News.find().forEach((article) => {
                expect(article).to.have.property('published_on');
            });
        });

        it('has a imageurl', () => {
            News.find().forEach((article) => {
                expect(article).to.have.property('imageurl');
            });
        });
        ...
        it('has a categories', () => {
            News.find().forEach((article) => {
                expect(article).to.have.property('categories');
            });
        });

        it('has a lang', () => {
            News.find().forEach((article) => {
                expect(article).to.have.property('lang');
            });
        });

        it('has a source_info', () => {
            News.find().forEach((article) => {
                expect(article).to.have.property('source_info');
            });
        });
